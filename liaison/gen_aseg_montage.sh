...
if [ $# -lt "1" ]; then
 ...
 echo " otherwise, it will be saved into the subjects original freesurfer"
...
if [ $# -eq 2 ]; then
	...
	OUTDIR=${SUBJECTS_DIR}/${1}
fi

# And there's no point overwriting good images
if [ -f ${OUTDIR}/${1}.png ]; then
	echo "     ${1}.png exists, skip it.";
	exit 0
fi
...
#echo "montaging ${IMGS}"
montage ${IMGS} -tile 4x3 -geometry 320x320+1+1 ${OUTDIR}/${1}.png
